...
import { Plant, CropStatus } from '@/lib/api';
import { Modal, Button, Select, Input, FormField } from '@/components/ui';

interface CreateCropFormData {
  plantId: string;
  datePlanted: string;
...
interface CreateCropModalProps {
  ...
  formData: CreateCropFormData;
  setFormData: (data: CreateCropFormData) => void;
  plants: Plant[];
}
...
export function CreateCropModal({ 
  ...
  setFormData, 
  plants 
}: CreateCropModalProps) {
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add New Crop Record" size="lg">
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <FormField label="Outcome">
          <Select
            ...
            onChange={(e) => setFormData({ ...formData, outcome: e.target.value as CreateCropFormData['outcome'] })}
          >
