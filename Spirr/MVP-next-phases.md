...
**Shipped:**
- **Test Simmer — A2 frost-tender caution + harness improvements B1–B5 (2026-06-21).** Acted on `sim/FINDINGS.md`. **A2 (product):** new optional `frostTender?: boolean` on `PlantInfo` (`types/index.ts`), set on the 10 warm-season crops in `plants.json` (tomat ×2, basilikum, agurk, paprika, gresskar, bønner, squash, mais, solsikke); new pure `isFrostTender()` + `frostTenderPlantOutCaution(plant, lastFrostDoy, doy)` in `sowWindow.ts` (kept store-free — date formatted inline, no seasonTimeline import); a soft amber caution above the box list in `SowBoxPicker` (new `caution?` prop), computed in `Seedlings.tsx`'s "Plant ut" flow; the sim mirrors it — `ObservedSeedling.frostRisk`, surfaced in `render.ts` + the gardener's `warnings`, and the driver's `plantOut` appends the caution to its result note. **B3 (drift, the biggest fix):** extracted the SowNowCard grouping logic into a shared clock-safe pure lib `src/lib/sowNowGroups.ts` (`groupSowNow`, `groupSuccession`, `harvestSoonForPlanting`, `groupHarvestSoon`) — `SowNowCard.tsx` and `sim/observe/snapshot.ts` now build sow-now + harvest-soon from the *same* functions (snapshot previously used `buildSeasonTimeline` while the screen used its own inline `harvestSoonForPlanting` that even read `new Date()` directly → real divergence, now gone). Plus `sim/browser-fidelity.ts`: replays a fixture, serializes the exact `gt_*` localStorage + expected card contents, and a `check` mode diffs scraped DOM (via `?simNow=` + Chrome DevTools MCP) vs the headless snapshot — self-tested pass/fail. **B2:** `add_custom_plant` driver verb (`sim/driver/schema.ts` validation + `actions.ts` → `useCustomPlantsStore.addPlant`), its key surfaced via new `ObservedGarden.customPlants` in `render.ts`. **B4:** sharpened key-vs-handle wording in the gardener system prompt. **B1:** a light `note`-nudge prompt line + new `sim/report/friction-summary.ts` that collates every run's `--judge` friction into `sim/report/out/FRICTION.md` (29 findings from the existing 6 judged runs). **B5:** froze the 5 remaining scenarios as replay fixtures (`sim/__tests__/fixtures/`, verified green first) and made `replay-determinism.test.ts` table-driven over all 6. **Verified:** `npm test` 36 green (4 files), `tsc --noEmit` + `eslint` clean. **User TODO (needs local Ollama):** re-run `npx tsx sim/run.ts --scenario all --model qwen2.5:7b --judge` to see the prompt changes (more notes, custom plants) take effect, then `friction-summary.ts`; and the live `browser-fidelity` browser pass. **Follow-up (same day):** added a descriptive **Sesongresultat** outcome block to the report (`sim/eval/outcome.ts` + `report.ts` `## Sammendrag`) — gardener actions (sown/plant-out/harvest/removed) + the A1 read (modne signaler vist vs faktisk høstet vs modne uhøstet ved sesongslutt), telemetry not pass/fail; over the frozen fixtures it shows the under-harvest gap clearly (midsummer: 6 ready signals, 1 harvested, 3 ripe at season end). Logged finding **A5** in `sim/FINDINGS.md`: `set_location` dead-ends on a missing postnummer (4065 absent from the 5132-entry dataset; the eager-beginner fixated 48×/48 errors) — a real coverage-gap + error-recovery UX signal. `npm test` 38 green.
...
- **Scenarios (all 6 from the design doc):** `precultivation-windowsill-feb` (walking skeleton), `first-time-empty-vestland-march` (no-location cold start), `multi-year-rotation-veteran` (seeded from `demo-garden.json` — vekstskifte + jordbær perennial), `direct-sow-vs-transplant` (seeded same-crop pair, distinct GDD anchors), `midsummer-harvest-rush` (Høst-snart + succession), `cold-station-wont-ripen` (Alta — won't-ripen flag + greenhouse GDD bonus). Building the last one surfaced a real observe-layer gap — won't-ripen crops have no harvest window so they never entered `harvestSoon`; now surfaced as their own "modner ikke ute her" signal.
...
**Deferred (M2/M3 tail):** export/import round-trip invariant; a scenario-specific GDD-anchor assertion for direct-vs-transplant (general identity invariants cover it today); tier-2 Chrome-DevTools browser fidelity check via `?simNow=`; extracting the last `.tsx` grouping logic (SowNowCard/GardenInsights) into pure libs to close the observation-drift gap.
